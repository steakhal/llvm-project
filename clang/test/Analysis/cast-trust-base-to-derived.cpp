...
template <class T> void clang_analyzer_DumpOverriderClasses(T mptr);
template <class ...Ts> void clang_analyzer_areRootClasses();
...
void top(Base &B) {
  ...
  // clang_analyzer_dump(B.number()); // expected-warning{{2}} expected-warning{{conj}}
  // clang_analyzer_areRootClasses<SuperBase, Base, Derived, Derived2, Derived3>();
  // // expected-warning@-1 {{[FALSE, FALSE, TRUE, TRUE, TRUE]}}

  // clang_analyzer_DumpOverriderClasses(&Base::number);
  // // expected-warning@-1 {{[BaseToDerivedRef::Derived, BaseToDerivedRef::Derived2, BaseToDerivedRef::SuperBase]}}

  // clang_analyzer_DumpOverriderClasses(&SuperBase::number);
  // // expected-warning@-1 {{[BaseToDerivedRef::Derived, BaseToDerivedRef::Derived2, BaseToDerivedRef::SuperBase]}}

  // clang_analyzer_DumpOverriderClasses(&Derived::number);
  // // expected-warning@-1 {{[BaseToDerivedRef::Derived]}}

  // clang_analyzer_DumpOverriderClasses(&Derived2::number);
  // // expected-warning@-1 {{[BaseToDerivedRef::Derived2]}}

  clang_analyzer_DumpOverriderClasses(&Derived3::number);
  // expected-warning@-1 {{[BaseToDerivedRef::Derived2]}}
}
} // namespace BaseToDerivedRef
...
void clang_analyzer_asdasd(const void *p);
...
struct A2 {
  virtual int number() const { return 3; }
};
