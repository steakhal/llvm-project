...
static SVal castArgToParamTypeIfNeeded(const CallEvent &Call,
  ...
  // the parameters at the runtime definition. Note that this is only ever
  // reached for prototype-less C functions, so the argument indices and the
  // declared parameter indices coincide.
...
static void addParameterValueToBindings(const StackFrame *CalleeSF,
  ...
  Bindings.push_back(std::make_pair(
      ParamLoc, processArgument(ArgVal, ArgExpr, ParamDecl, SVB)));
}
...
static void addParameterValuesToBindings(const StackFrame *CalleeSF,
  ...
  for (unsigned Idx = 0, NumArgs = Call.getNumArgs(); Idx != NumArgs; ++Idx) {
    ...
    // simply do not bind any values to them. The indices are monotonic, so
    // none of the remaining arguments has a parameter either.
