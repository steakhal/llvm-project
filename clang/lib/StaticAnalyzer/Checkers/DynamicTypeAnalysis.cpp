...
static MethodVec getOverridersImpl(DynamicTypeAnalysisImpl &Analysis,
  ...
  // Ignore methods that are in a dependent context because they are effectively templates.
  llvm::erase_if(CacheSlot->second, std::mem_fn(&CXXMethodDecl::isDependentContext));
  return CacheSlot->second;
}
